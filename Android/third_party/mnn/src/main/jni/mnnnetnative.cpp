...
extern "C" JNIEXPORT jlong JNICALL
Java_com_taobao_android_mnn_MNNNetNative_nativeCreateNetFromBuffer(JNIEnv *env, jclass type, jbyteArray bytes_, jint size_) {
    jbyte *dates = env->GetByteArrayElements(bytes_, NULL);
    auto interpreter      = MNN::Interpreter::createFromBuffer(dates,size_);
    env->ReleaseByteArrayElements(bytes_,dates,0);
    return (jlong)interpreter;
}
