...
void aoe_MD5Update(aoe_MD5_CTX *context, unsigned char *input, unsigned int inputlen)
{
	...
	if(inputlen >= partlen)
	{
		...
		aoe_MD5Transform(context->state, context->buffer);

		for(i = partlen; i+64 <= inputlen; i+=64)
			aoe_MD5Transform(context->state, &input[i]);
...
void aoe_MD5Final(aoe_MD5_CTX *context, unsigned char digest[16])
{
	...
	aoe_MD5Encode(bits, context->count, 8);
	aoe_MD5Update(context, PADDING, padlen);
	aoe_MD5Update(context, bits, 8);
	aoe_MD5Encode(digest, context->state, 16);
}
